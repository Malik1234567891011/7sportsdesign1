...
document.addEventListener("DOMContentLoaded", function () {
    ...
    const programs = [
        ...
        {
            id: 7,
            name: "Pierrefonds Multi-Program Center",
            sport: "multisport",
            city: "Pointe-Claire",
            lat: 45.47,
            lng: -73.75,
            ...
            season: ["fall", "winter", "spring", "summer"],
            description:
                "Ages 2-12 • Multiple programs available • All seasons",
            multiProgram: true,
            availablePrograms: ["soccer", "hockey", "multisport"],
        },
    ];
    ...
    const filterPrograms = (e) => {
        ...
        let filtered = programs.filter((program) => {
            ...
            // Sport filter - handle multi-program locations
            if (sport) {
                if (program.multiProgram) {
                    // For multi-program locations, check if sport is in availablePrograms
                    if (
                        !program.availablePrograms ||
                        !program.availablePrograms.includes(sport)
                    ) {
                        match = false;
                    }
                } else if (program.sport !== sport) {
                    match = false;
                }
            }
            ...
            // City filter
            if (city && program.city !== city) {
                match = false;
            }

            // Season filter
            if (season && program.season && !program.season.includes(season)) {
                match = false;
            }
    ...
    const updateResultsList = (programsToShow) => {
        ...
        resultsList.innerHTML = programsToShow
            .map((program) => {
                // Handle multi-program locations
                const badgeText = program.multiProgram
                    ? "Multiple Programs"
                    : program.sport.charAt(0).toUpperCase() +
                      program.sport.slice(1);
                const badgeClass = program.multiProgram
                    ? "multi-program"
                    : program.sport;
                const seasonText = program.season
                    ? program.season
                          .map((s) => s.charAt(0).toUpperCase() + s.slice(1))
                          .join(", ")
                    : "All Seasons";
                const programsList =
                    program.multiProgram && program.availablePrograms
                        ? `<p class="result-programs"><strong>Available:</strong> ${program.availablePrograms
                              .map(
                ...
                return `
            <div class="result-card" data-lat="${program.lat}" data-lng="${
                    program.lng
                }" data-sport="${program.sport || "multi"}">
                <div class="result-image" style="background: ${
                    program.multiProgram
                        ? "linear-gradient(135deg, #D62828 0%, #2B59C3 50%, #F6C443 100%)"
                        : gradients[program.sport]
                };"></div>
                <div class="result-content">
                    ...
                    <p class="result-season"><strong>Seasons:</strong> ${seasonText}</p>
                    <div class="result-footer">
                        <span class="result-price">From $${
                            program.price
                        }/month</span>
        ...
        newCards.forEach((card, index) => {
            card.addEventListener("click", (e) => {
                ...
                const markerIndex = programs.findIndex(
                    (p) => p.id === program.id
                );
    ...
    if (sportParam || ageParam || cityParam || seasonParam) {
        filterPrograms();
    }
});
